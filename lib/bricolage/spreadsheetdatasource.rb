...
module Bricolage
  ...
  class SpreadsheetDataSource < DataSource
    ...
    DEFAULT_APPLICATION_NAME = 'BricolageJob'

    def initialize(credentials: nil, scope: nil, application_name: nil)
      @credentials = credentials
      @scope = eval('Google::Apis::SheetsV4::' + (scope || DEFAULT_SCOPE))
      @application_name = application_name || DEFAULT_APPLICATION_NAME
    end

    attr_reader :credential_file_path, :scope, :application_name
